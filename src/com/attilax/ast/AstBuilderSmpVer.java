...
import com.attilax.orm.AtiOrmV2;
import com.attilax.ref.refx;
import com.google.common.collect.Lists;

public class AstBuilderSmpVer {
	public Object obj;
	public Object rzt;
	...
	public static void main(String[] args) throws UnsupportedEncodingException {
		...
		Object rzt22 =	IocFacV3_iocx_iocutil.getBean(AtiOrmV2.class).queryAsRzt("select sum(rmb) sumx from recharge where accountId in (  select id as uid from account where promoter=2981 )");
		
		//t3(s);
		String dsl="com.attilax.ioc.IocFacV3_iocx_iocutil.getBean(com.attilax.orm.AtiOrmV2.class).queryAsRzt('select sum(rmb) sumx from recharge where accountId in (  select id as uid from account where promoter=2981 )':s)";
		List<Token> tokens = new JavaTokener(dsl).getTokensV4();// er(args).getTokens();
	...
	private static Object t3(String s) {
		Expression buildAst = new AstBuilderSmpVer().buildAst(s);
...
//			return buildAstV2_params(tokens2);
	...
	//	Token nextToken = getNext2JoinToken();	
		...
		if(hasTokenTypeTag())
		{
			varType=getTokenTye();  ///  s str,i con
			m_index=m_index+2;   //skip two index...
		}
		else
			varType=cur.Type;
		
		if(varType.equals("s") ||  varType.equals("str")  )
			params.add(cur.Text);
		else if(varType.equals("con") ||  varType.equals("i")  )
	...
	/**
	attilax    2016年10月27日  下午5:50:19
	 * @return
	 */
	private String getTokenTye() {
		Token next= tokens.get(m_index+1);
		if(next.Text.equals(":") && next.Type.equals("op"))
		{
			String tokentype=tokens.get(m_index+2).Text;
			return  tokentype;
		}
			return null;
	}
	...
	/**
	attilax    2016年10月27日  下午5:50:17
	 * @return
	 */
	private boolean hasTokenTypeTag() {
		Token next= tokens.get(m_index+1);
		return (next.Text.equals(":") && next.Type.equals("op"));
