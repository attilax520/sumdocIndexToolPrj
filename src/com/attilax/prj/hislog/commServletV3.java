 ...
 *
 */
//http://localhost:1314/webcon/commServletV3?class= com.attilax.web.UploadServlet1  &method=upProcessInfo &argstypes=&args=
...
@WebServlet(name = "commServletV3", urlPatterns = { "/commServletV3" }, loadOnStartup = 1)
public class commServletV3 extends HttpServlet {
	...
	private List<Object> getParams(HttpServletRequest request) throws Exception {
		...
		if (request.getParameter("args").length() > 0) {
			...
			String paramtypes = request.getParameter("argstypes");
			String[] a = arguments.split(",");
			int n = 0;
			for (String arg1 : a) {
				Object p = refServiceV2s530.getArg(arg1, n, paramtypes.split(","));
