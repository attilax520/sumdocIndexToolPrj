...
public class dirx {
	...
	private static void tt() {
		...
		dirx.traveDirPa1(d, new com.attilax.lang.Closure<String, String>() {
	 ...
	 */
	@Deprecated
	public static void traveDir(String strPath ,com.attilax.lang.Closure  closure) { 
	    ...
	    if(isLastLevDir(dir))
			try {
				closure.execute(dir.getAbsolutePath());
			} catch (Exception e) {
		...
		else
	    {
	    	for (int i = 0; i < files.length; i++) { 
		        if (files[i].isDirectory()) { 
		        	try {
		        		traveDir(files[i].getAbsolutePath(),closure); 
					} catch (Exception e) {
						e.printStackTrace();
					}
...
public static void traveQ21(String strPath ,Closure closure) { 
		
		trave(strPath,closure);//q0f remove try catch
		...
		for (int i = 0; i < length; i++) { 
	        if (files[i].isDirectory()) { 
	            ...
	            try {
					...
					throw  new RuntimeException("strFileName:"+strFileName,e);
				}  
	...
	/**
	 * trave and use trave mode
	 * @param strPath
	 * @param closure
	 */
	@Deprecated 
	public   void traveFile_noIncDir(String strPath ,com.attilax.lang.Closure  closure) { 
	    File dir = new File(strPath); 
	    File[] files = dir.listFiles(); 
		...
		for (int i = 0; i < length; i++) { 
	        if (files[i].isDirectory()) { 
	        	traveFile_noIncDir(files[i].getAbsolutePath(),closure); 
	        } else { 
	            String strFileName = files[i].getAbsolutePath().toLowerCase();
	            ...
	            try {
					...
					if(traveCanBreakThrowEx)
					    core.throwRuntimeException(e);
					else
						e.printStackTrace();
						//throw  new RuntimeException("",e);
				}  
	...
	public dirx setTraveMode(String traveMode2) {
